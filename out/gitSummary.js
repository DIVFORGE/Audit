...
exports.insertSummaryIntoCommitMessage = insertSummaryIntoCommitMessage;
const vscode = require("vscode");
const fs = require("fs");
function summarizeLog(content) {
    ...
    for (const block of blocks) {
        ...
        if (/^- Mode: AI —/m.test(block))
            aiCount++;
        else if (/^- Mode: Developer —/m.test(block))
            devCount++;
        if (/^- Design-Change: true/m.test(block))
            designChangeCount++;
    }
    return {
...
function formatSummaryLine(summary) {
    ...
    if (summary.aiCount > 0)
        parts.push(`${summary.aiCount} AI-assisted edit${summary.aiCount === 1 ? '' : 's'}`);
    if (summary.devCount > 0)
        parts.push(`${summary.devCount} manual edit${summary.devCount === 1 ? '' : 's'}`);
 ...
 */
async function insertSummaryIntoCommitMessage(todayLogPath) {
...
//# sourceMappingURL=gitSummary.js.map
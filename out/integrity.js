...
exports.noteWrite = noteWrite;
exports.registerIntegrityProtection = registerIntegrityProtection;
 ...
 */
function noteWrite(logFilePath, fullContent) {
    lastKnownGood.set(logFilePath, fullContent);
}
function restorationNoticeBlock(reason) {
...
function restore(fsPath, reason) {
    const cached = lastKnownGood.get(fsPath);
    if (cached === undefined)
        return; // no baseline yet — nothing we can restore
    try {
        ...
        vscode.window.showWarningMessage(`Audit: today's log was modified outside the extension (${reason}) — restored automatically.`);
    }
    catch {
        /* best effort — if we can't write, there's nothing more to do here */
    }
}
function checkIntegrity(fsPath) {
    const cached = lastKnownGood.get(fsPath);
    if (cached === undefined)
        return; // we never wrote this file — not ours to protect
 ...
 */
function registerIntegrityProtection(context) {
    const config = vscode.workspace.getConfiguration('audit');
    if (!config.get('protectLogFromExternalChanges', true))
        return;

...
import { parseAllLogs, LogEntry } from './logParser';
import { getLogFolderPath } from './logger';

export function openDashboard(workspaceRootFsPath?: string) {
...
function renderDashboardHtml(entries: LogEntry[]): string {
  ...
  const rows = entries
    ...
    .map(
      e => `
      <tr data-category="${e.category}" data-area="${escapeHtml(e.area.toLowerCase())}" data-language="${escapeHtml(
        e.language.toLowerCase()
      )}" data-file="${escapeHtml(e.file.toLowerCase())}" data-who="${escapeHtml(e.toolOrDeveloper.toLowerCase())}">
        <td>${e.date} ${e.time}</td>
